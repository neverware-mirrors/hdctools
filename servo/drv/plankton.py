...
class plankton(pty_driver.ptyDriver):
  ...
  def _get_ina(self):
    matches = self._issue_cmd_get_results(self.INA_SENSE,
                                          self.INA_SENSE_DICT.values())
    return dict(zip(self.INA_SENSE_DICT.keys(), zip(*matches)[1]))
  ...
  def _i2c_read(self, bus, addr, offset):
    match = self._issue_cmd_get_results(
        'i2cxfer r %d 0x%x 0x%x' % (bus, addr, offset), [r'\S+ \[(\d+)\]'])[0]
