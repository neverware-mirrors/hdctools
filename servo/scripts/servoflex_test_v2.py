...
def do_cmd(cmd, timeout, plist=None, flist=None):
  ...
  if type(cmd) is str:
    cmd = cmd.split()
  cmd_obj = subprocess.Popen(cmd, 0, None, None, subprocess.PIPE,
                             subprocess.PIPE)
  ...
  while (time.time() - start_time) < timeout:
    if plist or flist:
      (rfds, _, _) = select.select([cmd_obj.stdout, cmd_obj.stderr], [], [],
                                   0.01)
      if len(rfds) > 0:
        ...
        if plist:
          for pass_str in plist:
            if pass_str in log_str:
              logging.debug('FOUND: %s in %s', pass_str, log_str)
              retval = True
...
FLASHROM_PASS = [
    'probe_spi_rems: id1 0xbf, id2 0x48', 'Found Generic flash chip'
]
FLASHROM_FAIL = None
...
def test_uart(dev_id, options):
  ...
  if not errors:
    ...
    if rsp_str != send_str:
      logging.error('Sent(%s) != Rcv(%s) for UART %s', send_str, rsp_str,
                    id_str)
      errors += 1
    os.close(fd)
...
def test_gpios(options):
  ...
  errors = 0
  cmd = [
      'spi2_vref:{pwr}', 'sd_vref_sel:{pwr}', 'sd_en:{val}', 'fw_wp_en:{val}',
      'jtag_vref_sel1:{pwr}', 'jtag_vref_sel0:{pwr}'
  ]
  ...
  for _ in range(2):
    for set_name, get_name in gpios_to_test.items():
      ...
      logging.debug('Trying %s %s -> %s', set_name, all_ctrls[set_name],
                    set_val)
      (retval, ctrl) = get_ctrls('%s:%s %s' % (set_name, set_val, get_name))
      if not retval:
        logging.error('Getting GPIO %s', get_name)
        errors += 1
      else:
        if ctrl[get_name] != set_val:
          logging.error('GPIO %s from %s -> %s', set_name, all_ctrls[set_name],
                        set_val)
          errors += 1
        else:
